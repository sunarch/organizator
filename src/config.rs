...
fn load_data_dir_single(config_dir: &Path, config_file_name: &str, config_file_purpose: &str) -> PathBuf {
    ...
    if config_file_path.try_exists().expect("Checking config file existence unsuccessful.") {
        let mut file = match File::open(config_file_path.clone()) {
            Err(why) => {
                panic!("Couldn't open config file for {} ({})\n{}", config_file_purpose, config_file_path.clone().display(), why);
            },
            Ok(file) => file,
        };
    ...
    if !config_content_is_ok {
        ...
        loop {
            ...
            stdin.read_line(input).expect("Reading from input unsuccessful.");
            ...
            if data_dir.exists() {
                ...
                let data_dir_str: &str = match data_dir.to_str() {
                    None => {
                        panic!("Couldn't convert to string: '{}'", data_dir.clone().display());
                    }
                    Some(data_dir_str) => { data_dir_str }
                };

                if let Err(why) = file.write_all(data_dir_str.as_bytes()) {
                    panic!("Couldn't write to config file for {} ({})\n{}", config_file_purpose, config_file_path.clone().display(), why);
                }
...
pub fn load_data_dirs() -> (PathBuf, PathBuf, PathBuf) {
    ...
    let data_dir_todo: PathBuf = load_data_dir_single(&config_dir, "data-dir-path-todo.txt", "ToDo");
    let data_dir_todo_output: PathBuf = load_data_dir_single(&config_dir, "data-dir-path-todo-output.txt", "ToDo output");
    let data_dir_finances: PathBuf = load_data_dir_single(&config_dir, "data-dir-path-finances.txt", "finances");
